...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    val charCount: List[(Char,Int)] = chars.map(char => (char, 1))
    val Counted:Map[Char,List[(Char,Int)]] = charCount.groupBy(_._1)
    val Grouped:Map[Char,Int] = Counted.map(elem => wordCount(elem))
    Grouped.toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    val OrderedLeaf = freqs.sortBy(_._2)
    val ans = List[Leaf]()
    makeLeafList(OrderedLeaf,ans)
   ...
   */
  def until(pred: List[CodeTree] => Boolean , oprate: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = if(pred(trees)) trees else until(pred, oprate)(oprate(trees))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    val ans = List[Char]()
    anxDecode(tree, tree, bits, ans)
  }

  def anxDecode(tree: CodeTree, curTree: CodeTree, bit: List[Bit],ans: List[Char]): List[Char] ={
    if(bit.isEmpty)
    {
      curTree match
      {
    ...
    else
    {
      curTree match
      {
        case Fork(left,right,c,w) =>
          bit.head match {
            case 0 => anxDecode(tree, left, bit.tail, ans)
            case 1 => anxDecode(tree, right, bit.tail, ans)
          }
        case Leaf(c,w) => anxDecode(tree, tree, bit, ans :+ c)
      }
    }
  }
   ...
   */
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = a.toSet.union(b.toSet).toList
